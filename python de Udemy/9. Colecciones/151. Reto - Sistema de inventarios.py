...
for indice in range(numero_productos):
    print(f"Proporciona los valores del producto {indice}")
    nombre = input("Nombre: ")
...
if 0 <= id_buscar <= numero_productos:
    print(f"""\nDetalles del producto con id {id_buscar}:
    ID: {inventario[id_buscar].get("id")}
        Nombre: {inventario[id_buscar].get("nombre")}.
        Precio: ${inventario[id_buscar].get("precio"):.2f}
        Cantidad: {inventario[id_buscar].get("cantidad")}""")
else:
